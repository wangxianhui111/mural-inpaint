...
class GatedGenerator(nn.Module):
    def __init__(self, opt):
        ...
        self.context_attention = ContextualAttention(ksize=3, stride=1, rate=2, fuse_k=3, softmax_scale=10,
                                                     fuse=True)
        # self.CBAM = CBAMBlock("FC", 7, channels = opt.latent_channels * 4, ratio = 9)
    ...
    def forward(self, img, mask,sketch):
        ...
        first_in = torch.cat((first_masked_img, mask, sketch), dim=1)
        ...
        first_out1 = torch.cat([first_out , mask, sketch], dim=1)
        refine_atten = self.refine_atten_1(second_in)
        refine_atten2 = self.refine_atten_1(first_out1)
        mask_s = nn.functional.interpolate(mask, (refine_atten.shape[2], refine_atten.shape[3]))
        sketchs = (1 - (mask) + (1 - sketch) * (mask))
        refine_atten = self.context_attention(refine_atten2, refine_atten, mask_s,sketchs)
...
class PerceptualNet(nn.Module):
    def __init__(self):
        ...
        block = [torchvision.models.vgg16(pretrained=True,progress= True).features[:15].eval()]
        for p in block[0]:
    ...
    def forward(self, x):
        ...
        x = self.transform(x, mode='bilinear', size=(256, 256), align_corners=False)
